              ...
              const ndn::ConstBufferPtr& keyLocatorHash)
{
  ...
  int size = sizeof(fullName)+sizeof(keyLocatorHash)+sizeof(id);
  std::cout<<"key locator hash"<<sizeof(keyLocatorHash)<<" name "<<sizeof(fullName)<<" size "<<sizeof(id)<<" total "<<sizeof(entry)<<std::endl;
  std::cout<<"entry size "<<sizeof(entry)<<" name "<<fullName<<std::endl;
  Name test("/test/1000");
  Entry testEntry(test, keyLocatorHash, id);
  std::cout<<"test name size "<<sizeof(testEntry)<<std::endl;
  bool isInserted = m_skipList.insert(entry).second;
  if (isInserted)
    ++m_size;
  return isInserted;
