...
SkipList<T, Compare, Traits>::erase(typename SkipList<T, Compare, Traits>::const_iterator it)
{
  ...
  if (!empty() && eraseNode != m_head) {
    ...
    for (int i = nLevels - 1; i >= 0; --i) {
      ...
      typename std::list<SkipListNodePointer>::iterator it_erase_next_prev = (*it_erase_next)->nexts.begin();
      for (int j = 0; j < i; j++) {
        it_erase_next_prev++;
      }
      ...
      *it_erase_next_prev = *it_erase_prev;
      ...
      typename std::list<SkipListNodePointer>::iterator it_erase_prev_next = (*it_erase_prev)->prevs.begin();
      for (int j = 0; j < i; j++) {
        it_erase_prev_next++;
      }
